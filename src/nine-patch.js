export default class NinePatch {
   ...
   */
  async init() {
    this._rawContext = await getOriginalImgContext(this._srcUrl);
    /* 获取图片的宽高
  ...
  draw(element) {
    ...
    } else element.style.backgroundImage = `url('${this._srcUrl}')`;
  }
  ...
  getBase64(width, height) {
    ...
    if (this._horizontalPatch && this._verticalPatch) {
      ...
      const horizontalStretchRatio = (width - this._fixWidth) / (this._imageWidth - this._fixWidth);
      const verticalStretchRatio = (height - this._fixHeight) / (this._imageHeight - this._fixHeight);
      ...
      for (let row = 0; row < this._verticalPatch.length; ++row) {
        /* 将临时画布改为该行的 patch 高度
        ...
        for (let col = 0; col < this._horizontalPatch.length; ++col) {
          /* 将临时画布改为该 patch 宽度
