/**
 * 绘制 9 patch 图片
 * Draw a 9 patch picture
 *
 * @param element 被绘制的元素 The element to be drawn
 * @param srcUrl 图片地址 The image address
 */
export async function draw9PatchImage(element, srcUrl) {
  const width = element.clientWidth
  const height = element.clientHeight
  const rawContext = await getRawImgContext(srcUrl)
  element.style.backgroundSize = `${width}px ${height}px`
  element.style.backgroundRepeat = 'no-repeat'
  element.style.backgroundImage = `url('${await (get9PatchBase64(rawContext, width, height)) ?? srcUrl}')`
  const padding = getPadding(rawContext)
  if (padding[0] || padding[1] || padding[2] || padding[3]) {
    element.style.padding = `${padding[0]}px ${padding[1]}px ${padding[2]}px ${padding[3]}px`
    element.style.boxSizing = 'border-box'
  }
}

/**
 * 获取 9 patch 图片的 base64 url
 * Get the base64 or url of the 9 patch image
 *
 * @param srcUrl 图片地址 The image address
 * @param width 宽度
 * @param height 高度
 * @returns base64 url base64 url
 */
export async function get9PatchBase64Url(srcUrl, width, height) {
  get9PatchBase64(await getRawImgContext(srcUrl), width, height)
}
 ...
 *
 * @param rawContext 原始图片上下文 The original image context
 * @param width 宽度
 * @param height 高度
 * @returns base64
 */
async function get9PatchBase64(rawContext, width, height) {
  /* 获取图片的宽高
  Get the width and height of the image */
  const imageWidth = rawContext.canvas.width - 2
  const imageHeight = rawContext.canvas.height - 2
  /* 获取 patch 数据
   * Get patch data */
  const horizontalPatch = getHorizontalPatch(rawContext)
  const verticalPatch = getVerticalPatch(rawContext)
  ...
  if (horizontalPatch && verticalPatch) {
    ...
    *  In the original image, the width and height length that will not be stretched is the fixed length */
    const fixWidth = horizontalPatch.reduce((acc, cur) => acc + (cur < 0 ? -cur : 0), 0)
    const fixHeight = verticalPatch.reduce((acc, cur) => acc + (cur < 0 ? -cur : 0), 0)
    ...
    const horizontalStretchRatio = (width - fixWidth) / (imageWidth - fixWidth)
    const verticalStretchRatio = (height - fixHeight) / (imageHeight - fixHeight)
    ...
    for (let row = 0; row < verticalPatch.length; ++row) {
      /* 将临时画布改为该行的 patch 高度
       * Change the temporary canvas to the patch height of the row */
      tempCanvas.height = Math.abs(verticalPatch[row])
      /* 遍历该行的每一列 patch，即每一个单独的 patch
       * Iterate through each column of patches in the row, i.e., each individual patch */
      for (let col = 0; col < horizontalPatch.length; ++col) {
        /* 将临时画布改为该 patch 宽度
         * Change the temporary canvas to that patch width  */
        tempCanvas.width = Math.abs(horizontalPatch[col])

        /* 将原图的 patch 绘制到临时画布
         * Draw the patch of the original image to the temporary canvas */
        tempContext.drawImage(rawContext.canvas, sx, sy, tempCanvas.width, tempCanvas.height, 0, 0, tempCanvas.width, tempCanvas.height)
        ...
        dw = horizontalPatch[col] < 0 ? -horizontalPatch[col] : horizontalPatch[col] * Math.max(horizontalStretchRatio, 0)
        dh = verticalPatch[row] < 0 ? -verticalPatch[row] : verticalPatch[row] * Math.max(verticalStretchRatio, 0)
        /* 将临时画布的 patch 绘制到目标画布
         * Draw a patch of the temporary canvas to the target canvas */
        context.drawImage(tempCanvas, 0, 0, tempCanvas.width, tempCanvas.height, dx, dy, dw, dh)
        /* 更新目标画布的 patch 位置
 ...
 */
function getRawImgContext(srcImg) {
